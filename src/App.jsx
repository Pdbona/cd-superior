...
/* Tela do Coletor — o app do Conferente (registro de início/fim de operação
   no celular). Não é uma aba de TABS_GESTOR porque não é um painel dentro do
   Gestor; é outro app inteiro (AppConferente). Fica de fora dessa lista mas
   entra como mais uma opção no checklist de acessos de qualquer perfil —
   inclusive Gestor, que combinado com Pablo em 16/ago/2026 passa a poder
...
/* Telas que só o Administrador libera para um perfil — nunca aparecem como
   ...
   poderia criar um perfil próprio com acesso a Parâmetros/Perfis e, na
   prática, fabricar um "gestor" novo por fora do cadastro exclusivo do
   Administrador. Combinado com Pablo em 16/ago/2026. */
const TELAS_RESTRITAS_ADMIN = ["parametros", "perfis"];
  ...
  const carregar = useCallback(async (silencioso) => {
    ...
    try {
      ...
      if (p?.value) {
        ...
        /* idem para conferentes e permissoesGestor — nunca tiveram esse
           reforço; sem ele, um documento salvo sem essas chaves (ex.: por um
...
/* ============================================================
   ...
   Porta de entrada de mais alto nível. Combinado com Pablo em 20/ago/2026:
   Comercial e Administrativo saíram daqui — agora só existem como atalho
   dentro do painel (sidebar do AppGestor, ver podeComercial/
   podeAdministrativo), depois que a pessoa já entrou pela Porta de
   Entrada normal (Gestor/Conferente/Perfil). Evita pedir usuário+senha
   duas vezes. ADMIN do Sistema fica à parte, num canto — é quem antes
   era o botão ADMINISTRADOR aqui dentro do Operacional.
   ============================================================ */
          ...
          {!pedirPinAdmin ? (
            <>
              <button style={hubBtn(false)} onClick={onOperacional}>
                ...
                <div style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 800, fontSize: 15, color: C.navy }}>APP_GESTÃO SUPERIOR CD</div>
              </button>
...
/* ============================================================
   ...
   ============================================================ */
function PortaEntrada({ params, persistParams, onVoltar, onEntrar }) {
  ...
  /* ---- "Esqueci meu PIN" — autoatendimento (combinado com Pablo em
     16/ago/2026): a pessoa se identifica pelo próprio nome (a mesma lista
     que o gestor/administrador já cadastrou) e define um PIN novo. Não
     existe e-mail/SMS neste app, então a verificação é o nome — mesmo
     nível de confiança que o app já usa para tudo mais (PIN sem usuário).
     Só existe onde há PIN individual: não serve para o PIN de emergência
     do Gestor (pinGestor) nem para o Administrador (fixo, no código). */
  const [modoRedefinir, setModoRedefinir] = useState(false);
  const [pessoaRedefId, setPessoaRedefId] = useState("");
  const [novoPin1, setNovoPin1] = useState("");
  const [novoPin2, setNovoPin2] = useState("");
  const [erroRedef, setErroRedef] = useState("");
  const [okRedef, setOkRedef] = useState(false);

  /* pessoa bloqueada não aparece na lista de redefinição — senão redefiniria
     o próprio PIN e voltaria a entrar, furando o bloqueio do Administrador */
  const listaRedefinir =
    pedirPin === "conferente" ? conferentes.filter(c => !c.bloqueado)
    : pedirPin === "gestor" ? gestores
    : pedirPin === "perfil" ? pessoasPerfil.filter(p => p.perfilId === perfilAlvo?.id && !p.bloqueado)
    : [];

  const abrirRedefinir = () => {
    setModoRedefinir(true); setPessoaRedefId(""); setNovoPin1(""); setNovoPin2(""); setErroRedef(""); setOkRedef(false);
  };
  const fecharRedefinir = () => { setModoRedefinir(false); setErroRedef(""); setOkRedef(false); };

  const redefinirPin = () => {
    if (!pessoaRedefId) return setErroRedef("Selecione seu nome.");
    if (!/^\d{4,6}$/.test(novoPin1)) return setErroRedef("O PIN deve ter de 4 a 6 dígitos numéricos.");
    if (novoPin1 !== novoPin2) return setErroRedef("Os dois PINs digitados são diferentes.");
    if (pinEmUsoGlobal(novoPin1, params, pessoaRedefId)) return setErroRedef("Este PIN já está em uso por outro acesso do app.");
    if (pedirPin === "conferente") {
      persistParams({ ...params, conferentes: conferentes.map(c => c.id === pessoaRedefId ? { ...c, pin: novoPin1 } : c) });
    } else if (pedirPin === "gestor") {
      persistParams({ ...params, gestores: gestores.map(g => g.id === pessoaRedefId ? { ...g, pin: novoPin1 } : g) });
    } else if (pedirPin === "perfil") {
      persistParams({ ...params, pessoasPerfil: pessoasPerfil.map(p => p.id === pessoaRedefId ? { ...p, pin: novoPin1 } : p) });
    }
    setErroRedef(""); setOkRedef(true);
    setTimeout(() => { fecharRedefinir(); setPin(""); }, 1800);
  };
  ...
  const validar = () => {
    ...
    if (achado?.bloqueado) { setErro("Acesso bloqueado. Procure o Administrador."); setPin(""); return; }
  ...
  const entrarConferente = () => {
    ...
    setPedirPin("conferente"); setPin(""); setErro(""); setModoRedefinir(false);
  };
      ...
      <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div style={{ width: "100%", maxWidth: pedirPin ? 460 : 860 }}>
          ...
          {!pedirPin ? (
            ...
            <div style={{ ...styles.card, textAlign: "center" }}>
              <Lock size={26} color={C.laranja} />
              <div style={{ fontFamily: "'Montserrat',sans-serif", fontWeight: 700, color: C.navy, margin: "10px 0 4px", fontSize: 15 }}>
                Esqueci meu PIN
              </div>
              {okRedef ? (
                <div style={{ ...styles.infoBox, marginTop: 14, textAlign: "left" }}>
                  <CheckCircle2 size={14} style={{ verticalAlign: -2, marginRight: 6, color: C.verde }} />
                  PIN redefinido! Volte e entre com o PIN novo.
                </div>
              ) : (
                <>
                  <div style={{ fontSize: 12.5, color: C.prata, marginBottom: 14 }}>
                    Escolha seu nome e defina um PIN novo.
                  </div>
                  <div style={{ textAlign: "left", marginBottom: 12 }}>
                    <Field label="Seu nome">
                      <select style={styles.input} value={pessoaRedefId}
                        onChange={e => { setPessoaRedefId(e.target.value); setErroRedef(""); }}>
                        <option value="">Selecione…</option>
                        {listaRedefinir.map(p => <option key={p.id} value={p.id}>{p.nome}</option>)}
                      </select>
                    </Field>
                  </div>
                  <div style={{ display: "flex", gap: 10, marginBottom: 4 }}>
                    <div style={{ flex: 1, textAlign: "left" }}>
                      <Field label="Novo PIN">
                        <input type="password" inputMode="numeric" maxLength={6} value={novoPin1}
                          onChange={e => { setNovoPin1(e.target.value.replace(/\D/g, "")); setErroRedef(""); }}
                          style={{ ...styles.input, textAlign: "center", fontFamily: "'Roboto Mono',monospace", letterSpacing: 4 }} />
                      </Field>
                    </div>
                    <div style={{ flex: 1, textAlign: "left" }}>
                      <Field label="Confirmar">
                        <input type="password" inputMode="numeric" maxLength={6} value={novoPin2}
                          onChange={e => { setNovoPin2(e.target.value.replace(/\D/g, "")); setErroRedef(""); }}
                          onKeyDown={e => e.key === "Enter" && redefinirPin()}
                          style={{ ...styles.input, textAlign: "center", fontFamily: "'Roboto Mono',monospace", letterSpacing: 4 }} />
                      </Field>
                    </div>
                  </div>
                  {erroRedef && <div style={{ color: C.vermelho, fontSize: 12.5, marginTop: 8, fontWeight: 600 }}>{erroRedef}</div>}
                </>
              )}
              <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
                <button style={{ ...styles.btnGhost, flex: 1, justifyContent: "center" }} onClick={fecharRedefinir}>
                  {okRedef ? "Voltar" : "Cancelar"}
                </button>
                {!okRedef && (
                  <button style={{ ...styles.btnPrimary, flex: 1, justifyContent: "center" }} onClick={redefinirPin}>Redefinir PIN</button>
                )}
              </div>
            </div>
          ) : (
            <div style={{ ...styles.card, textAlign: "center" }}>
              ...
              {listaRedefinir.length > 0 && (
                <button style={{ background: "none", border: "none", color: C.navy2, fontSize: 12, fontWeight: 600,
    ...
    <div style={styles.page}>
      ...
      <header style={{ ...styles.header, padding: "14px 18px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ ...styles.logoWrap, padding: "5px 8px" }}><img src={SUP_LOGO} alt="Superior Transportes" style={{ height: 24 }} /></div>
        </div>
      </header>
      <div style={styles.accentBar} />
...
function AppGestor({ ops, opsForecast, anonimizarCliente, params, persistOps, persistParams, diasTerc, persistDiasTerc, now, sair, sync, recarregar, usuario, permissoes, tituloPainel, sub }) {
  ...
  /* Tela do Coletor dentro do painel — combinado com Pablo em 16/ago/2026:
     ...
     "Tela do Coletor") abre o app do Conferente sem sair do painel. Mesma
     regra opt-out/opt-in do resto: Gestor de verdade só perde se alguém
  ...
  useEffect(() => {
    if (TABS.length === 0 && podeColetor) setModoColetor(true);
  }, [TABS.length, podeColetor]);

  if (modoColetor) {
    return <AppConferente ops={ops} params={params} persistOps={persistOps} now={now} usuario={usuario}
...
/* ============================================================
   APP DO ADMINISTRADOR — RBAC v2 (fase 5)
   ------------------------------------------------------------
   ...
   Cuida de duas coisas:
     1. Gestão de Acessos (GestaoAcessos, souAdministrador) — perfis
        próprios, Conferente e Gestor (inclusive o escopo de acesso do
        Gestor, que era a antiga seção solta "Telas do Gestor") e
        usuários, tudo numa lista só.
     2. Cadastro de PIN dos gestores + PIN de emergência — o único
        lugar do app onde isso é feito.
   ============================================================ */
function AppAdmin({ params, persistParams, usuario, sair, sync, recarregar }) {
  ...
  const salvar = () => {
    ...
    persistParams(clean);
    setDraft({ gestores: clean.gestores, pinGestor: clean.pinGestor });
    ...
    <div style={styles.page}>
      ...
      <main style={styles.main}>
        {/* Perfis e usuários vêm primeiro — é o que o Administrador abre a
           tela para fazer. O mesmo componente da aba "Perfis" do Gestor, só
           que aqui sem restrição de tela (o Administrador pode conceder
           Parâmetros e Perfis, o Gestor não) e com os cards de Conferente
           e Gestor liberados para edição — inclusive o escopo de acesso do
           Gestor, que substituiu a antiga seção solta "Telas do Gestor". */}
        <GestaoAcessos params={params} persistParams={persistParams} telasRestritas={[]} souAdministrador />
...
function GestaoAcessos({ params, persistParams, sub, telasRestritas = TELAS_RESTRITAS_ADMIN, souAdministrador = false }) {
  ...
  /* Conferente — perfil fixo (combinado com Pablo em 16/ago/2026): veio da
     aba Parâmetros pra cá, pra ficar junto de todo o resto de acesso. O
  ...
  /* Gestor — perfil fixo, acesso a tudo por padrão (inclusive a tela do
     coletor). Só o Administrador enxerga isso editável aqui; o cadastro de
     PIN dos gestores continua exclusivo dele, na tela do Administrador.
     Substitui a antiga seção solta "Telas do Gestor". */
  ...
  const addConferente = () => {
    ...
    if (draftConferentes.some(c => c.nome.toLowerCase() === nome.toLowerCase())) return setErroConf("Já existe um conferente com esse nome.");
    if (pinEmUsoGlobal(pin, { ...params, conferentes: draftConferentes })) return setErroConf("Este PIN já está em uso por outro acesso do app.");
    setDraftConferentes(d => [...d, { id: uid(), nome, pin }]);
    setNovoConf({ nome: "", pin: "" }); setErroConf(""); mudou();
  };
  const delConferente = (id) => { setDraftConferentes(d => d.filter(c => c.id !== id)); mudou(); };
  const setConfCampo = (id, campo, valor) => {
    setDraftConferentes(d => d.map(c => c.id === id ? { ...c, [campo]: valor } : c));
    mudou();
  };
  const alternarBloqueioConf = (id) => {
    setDraftConferentes(d => d.map(c => c.id === id ? { ...c, bloqueado: !c.bloqueado } : c));
    mudou();
  };
  ...
  const salvar = () => {
    ...
    const cleanConferentes = draftConferentes.filter(c => c.nome && c.pin)
      .map(c => {
        const base = { id: c.id || uid(), nome: c.nome.trim(), pin: c.pin.toString().trim() };
        if (c.bloqueado) base.bloqueado = true;
        return base;
      });
    const patch = { ...params, perfis: cleanPerfis, pessoasPerfil: cleanPessoas, conferentes: cleanConferentes };
    /* o escopo de acesso do Gestor só é editável (e só é salvo) pelo
  ...
  /* ---------------- editor de acessos (compartilhado) ----------------
     ...
     cliques. */
  const EditorAcessos = ({ telas, subMapa, onTela, onSub, semSubitens, somenteLeitura }) => (
    <div style={{ display: "grid", gap: 8 }}>
      {telasDisponiveis.map(t => {
        const ligada = !!telas?.[t.id];
        const subitens = semSubitens ? [] : (SUBITENS_POR_ABA[t.id] || []);
        const Ic = t.icon;
        return (
          <div key={t.id} style={{ border: `1px solid ${ligada ? C.navy2 : C.prataClaro}`, borderRadius: 9,
            ...
            <label style={{ display: "flex", alignItems: "center", gap: 9,
              ...
              <input type="checkbox" checked={ligada} disabled={somenteLeitura} onChange={() => onTela(t.id)} />
                  ...
                  <div style={{ display: "grid", gap: 8 }}>
                    {draftConferentes.map(c => (
                      <div key={c.id} style={{ ...styles.opRow, padding: "9px 10px", gap: 8, opacity: c.bloqueado ? .6 : 1 }}>
                        ...
                        <input style={{ ...styles.input, width: 70, textAlign: "center", fontFamily: "'Roboto Mono',monospace", letterSpacing: 1.5, padding: "6px 6px", fontSize: 13 }}
                          inputMode="numeric" maxLength={6} value={c.pin}
                          onChange={e => setConfCampo(c.id, "pin", e.target.value.replace(/\D/g, ""))} />
                        <button title={c.bloqueado ? "Desbloquear" : "Bloquear"}
                          onClick={() => alternarBloqueioConf(c.id)}
                          style={{ ...styles.iconBtnDanger, background: c.bloqueado ? "#FFEBEE" : "#EEF2F8", color: c.bloqueado ? C.vermelho : C.navy2 }}>
                          <Lock size={14} />
                        </button>
...
function Dashboard({ ops, opsForecast, anonimizarCliente, params, now, diasTerc, sub, subFotos, persistOps, usuario }) {
  ...
  /* Cancelamentos por Cliente e Fora da Meta ficam em sanfonas lado a
     lado, fechadas por padrão — evita poluir a tela com listas longas
     que não são o foco principal do dashboard. */
  const [secaoAberta, setSecaoAberta] = useState(null); // null | "cancelamentos" | "foraMeta"
  ...
  return (
    <div>
      ...
      {totalForecast > 0 && subOn("forecast") && (
        <>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(340px,1fr))", gap: 16, marginBottom: 22, alignItems: "start" }}>
          {cancelamentosCliente.length > 0 && subOn("cancelamentos") && (
            <Sanfona titulo="Cancelamentos por Cliente" icone={Building2} cor={C.vermelho}
              ...
              aberto={secaoAberta === "cancelamentos"}
              onToggle={() => setSecaoAberta(s => s === "cancelamentos" ? null : "cancelamentos")}>
          ...
          {foraMeta.length > 0 && subOn("forameta") && (
            <Sanfona titulo={`Operações Fora da Meta — ${nomeMes(mAtual)}`} icone={AlertTriangle} cor={C.vermelho}
              ...
              onToggle={() => setSecaoAberta(s => s === "foraMeta" ? null : "foraMeta")}>
              <div style={{ display: "grid", gap: 8 }}>
...
function Relatorios({ ops, params, diasTerc, sub, clienteRestrito }) {
  ...
  const mediaMesAnterior = useMemo(() => {
    const agora = new Date();
    const mesAnt = new Date(agora.getFullYear(), agora.getMonth() - 1, 1);
    const iniAnt = mesAnt.toISOString().slice(0, 10);
    const fimAnt = new Date(mesAnt.getFullYear(), mesAnt.getMonth() + 1, 0).toISOString().slice(0, 10);
    const opsAnt = ops
      .filter(o => o.status === "concluida" && o.fim >= new Date(iniAnt + "T00:00:00").getTime() && o.fim <= new Date(fimAnt + "T23:59:59").getTime())
      .filter(o => cliFiltro === "todos" || o.cliente === cliFiltro);
    const diasUnicos = new Set();
    let totalHoras = 0;
    opsAnt.forEach(op => {
      diasUnicos.add(op.fim ? new Date(op.fim).toISOString().slice(0, 10) : null);
      totalHoras += calcOp(op, params).tempoReal || 0;
    });
    return diasUnicos.size > 0 ? (totalHoras / diasUnicos.size) : 0;
...
function Parametros({ params, persistParams, persistOps, ops, sub }) {
  ...
  const salvar = () => {
    /* CORREÇÃO CRÍTICA (16/ago/2026): persistParams substitui o documento
       inteiro, não faz merge (ver App() → persistParams). Sem o "...params"
       aqui, qualquer campo que esta tela não edita — perfis, pessoasPerfil,
...
/* Rodapé padrão SBS — combinado com Pablo em 16/ago/2026: toda tela, exceto
...
function RodapeSBS({ direita }) {
  return (
    <footer style={styles.footer}>
      ...
      {direita && <div style={{ color: C.prata, textAlign: "right" }}>{direita}</div>}
    </footer>
  );
}
